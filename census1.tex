...
\begin{frame}
\frametitle{Text and Social Science}
...
\begin{frame}
\frametitle{Goal for this morning: Document-Term Matrices}
...
\begin{tiny}
stop\_words = urlopen('http://www.ai.mit.edu/projects/jmlr/papers/volume5/lewis04a/a11-smart-stop-list/english.stop').read().split('\\n') \\
\end{tiny}

\vspace{0.05in}

text\_4 = [x for x in text\_3 if x not in stop\_words]\\ 
\vspace{0.05in}


text\_rem = [x for x in text\_3 if x not in text\_4] \\
\end{tt}
...
\invisible<1-9>{Step 5: Create Count Vector  } \\
